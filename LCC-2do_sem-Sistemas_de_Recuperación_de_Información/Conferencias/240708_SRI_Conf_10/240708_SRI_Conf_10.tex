...
\begin{document}
	...
	\begin{frame}
		...
		\only<2>{
			\textcolor{purple}{¿Qué algoritmos \textbf{existen} para extraer información de un grafo, según lo estudiado durante la carrera?}
		}
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			% Esto puede ser porque por ellos pasan todo el tráfico de la red, o porque están aislados y contienen datos importantes de otros nodos.
	...
	\begin{frame}
		...
		\begin{minipage}{0.45\textwidth}
			...
			\only<2>{
				...
				% Si se parte de la recien y se aplica log por cada lado, se obtiene:
				%		log (pk) = -b*log(k) + log (a)
				% y al graficar eso queda una grafica 2D donde 
				%		y = log (pk)
				%		x = log (Degree)
				% la función es lineal, pero con la pendiente invertida.
	...
	\begin{frame}
		...
		\begin{minipage}{0.6\textwidth}
			...
			%	- los valores del diámetro aumentan.
	...
	\begin{frame}
		...
		\begin{itemize}
			...
			% Los grafos aleatorios son relativamente resistentes a fallos aleatorios, ya que la eliminación de nodos individuales tiende a tener un impacto limitado en la conectividad general de la red. Esto se debe a la falta de estructura organizada en el grafo.
			
			\item Ausencia de estructura de grupos densamente conectados
			% En un grafo aleatorio, no hay una estructura de comunidad clara o grupos densamente conectados de nodos. La formación de comunidades en un grafo aleatorio es puramente el resultado de la probabilidad aleatoria de conexión entre nodos.
	...
	%  	
	% Por cada arista, verifico su probabilidad de ser eliminada. 
	% En caso afirmativo, la elimino y coloco una arista entre uno de los vértices y otro generado por una distribución uniferme, estableciendo aristas de saltos.
